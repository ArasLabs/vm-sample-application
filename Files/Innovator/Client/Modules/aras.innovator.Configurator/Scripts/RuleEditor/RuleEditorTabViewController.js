...
function updateEditorGroupOptions() {
	...
	if (ruleEditor.isInputStarted()) {
		...
		targetGroup.setParserProperty('_FamilyNames', viewData.familyNameHash);
...
createUIControls = function() {
	require([
		...
		'Configurator/Scripts/RuleEditor/RuleTemplate',
		...
		viewContext.gridLayout = createGridLayout('cs_sample_Rule');
...
function createNewRuleItem() {
	return new Promise(function(resolve) {
		...
		var ruleModelItem = dataModel.createNewModelItem('cs_sample_Rule');
...
function showFilterDialog() {
	...
	var confirmDialogParams = {
		...
		content: '../Modules/aras.innovator.Configurator/Views/filterRuleDialog.html'
	};
	...
	viewContext.topWindow.ArasModules.Dialog.show('iframe', confirmDialogParams).promise.then(function(returnValue) {
		if (returnValue === 'ok') {
			...
			setRuleFilter(checkedItemsCount !== allModelItems.length - 1 ? groupItemsCache : null);
		}
	});
}
...
function showSelectedItemForm(forceShow) {
	...
	if (selectedTabId === targetTabId || forceShow) {
		...
		if (viewContext.selectedRuleItem) {
			...
			formViewHelper.showFormById('B237D9197A076F101249907013025199', isEditable ? 'edit' : 'view', ruleModelItem.node, tabDomNode, handleFormPropertyChange);
		} else {
...
function createGridItems(gridControlLayout) {
	var gridItems = [];
	var dataModel = viewContext.data.dataModel;
	var ruleItems = dataModel.getItemsByParameters({itemType: 'cs_sample_Rule'});
