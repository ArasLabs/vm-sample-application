...
function(declare) {
	return declare(null, {
		...
		_requestAggregatedSampleData: function(scopeId, optionalParameters) {
			...
			if (scopeId) {
				var scopeCacheId = this.aras.calcMD5(scopeId + (optionalParameters.builderMethodName || 'cs_sample_scopeBuilder'));
				...
				if (!requestData || optionalParameters.forceLoad) {
					...
					var targetScope = aras.newIOMItem('cfg_Scope', optionalParameters.builderMethodName || 'cs_sample_scopeBuilder');
